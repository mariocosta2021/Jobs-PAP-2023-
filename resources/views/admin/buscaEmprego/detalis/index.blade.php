    ...
    <form  method="POST" action="{{ route('admin.buscar.store') }}"   enctype="multipart/form-data">
        @csrf
                <div class="col-lg-9 d-md-flex">
                    <input name="emprego" class="form-control" type="text" placeholder="Procurar">
                        ...
                        <div class="card-body">
                            <div class="media mb-2">
                                <div class="media-body">
                                    <div  style='background-image:url("/storage/{{ $item->imagemEmprego}}");background-position:center;background-size:cover;height:200px;width:auto'>
                                    </div>
                                    <h4 class="fs-5 text-black mt-4">
                                        <a href="{{ route('admin.buscar.detalhes',$item->id) }}">{{ $item->tituloEmprego }}</a>
                                    </h4>
                                    <p class="mb-1">Empresa: <span class=" fw-bold">{{ $item->nomeEmresa }}</span></p>
                                </div>
                            </div>

                            <div class="d-flex align-items-center mt-4">
                                <a href="{{ route('admin.buscar.detalhes',$item->id) }}" class="btn btn-primary w-100"> Saber mais</a>
                            </div>
                        </div>
                    </div>
                </div>
